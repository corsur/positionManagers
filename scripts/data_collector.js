...
import { MetricHelper } from "./metric_helper.js";
...
const metricHelper = new MetricHelper(region);
...
const DB_LATEST_STRATEGY_TVL_WRITE_FAILURE =
  "DB_LATEST_STRATEGY_TVL_WRITE_FAILURE";
const DB_LATEST_STRATEGY_TVL_WRITE_SUCCESS =
  "DB_LATEST_STRATEGY_TVL_WRITE_SUCCESS";
...
async function run_pipeline() {
  ...
  const { network, qps, hive_batch_size } = parser.parse_args();
  blockchain_network = network;

  // Signal run status.
  metricHelper.plusOne(DATA_COLLECTOR_START);
  ...
  try {
    ...
    metricHelper.plusOne(GET_NEXT_POSITION_ID_FAILURE);
    return;
  }
  ...
  try {
    ...
    metricHelper.plusOne(GET_POSITION_MANAGER_FAILURE);
    return;
  }
  ...
  const resolved_promises = position_infos_promise
    .filter((x) => x.items[0].info.detailed_info !== null)
    .map((x) => {
      const { info, position } = x.items[0];
  ...
  for (const [index, raw_item] of resolved_promises.entries()) {
    ...
    if (
      ...
      try {
        ...
        metricHelper.plusN(TOTAL_POSITION_COVERED, position_ticks_items.length);
        metricHelper.plusOne(DB_POSITION_TICKS_WRITE_SUCCESS);
        console.log("Position ticks batch write is successful.");
      } catch (error) {
        console.error(
          `Failed to batch write for position ticks with error: ${error}`
        );
        metricHelper.plusOne(DB_POSITION_TICKS_WRITE_FAILURE);
      } finally {
  ...
  for (var strategy_id in mAssetToTVL) {
    ...
    await write_latest_strategy_metrics(
      latest_strategy_tvl_table,
      strategy_id,
      tvl_uusd
    );
  }
...
async function write_strategy_metrics(table_name, strategy_id, tvl_uusd) {
  const command = new PutItemCommand(
    strategy_input(table_name, strategy_id, tvl_uusd)
  );
  try {
    await client.send(command);
    console.log(`Strategy TVL write is successful.`);
    metricHelper.plusOne(DB_STRATEGY_TVL_WRITE_SUCCESS);
  } catch (err) {
    console.error(`Strategy TVL write failed with error:  ${err}`);
    metricHelper.plusOne(DB_STRATEGY_TVL_WRITE_FAILURE);
  }
}

async function write_latest_strategy_metrics(
  table_name,
  strategy_id,
  tvl_uusd
) {
  ...
  try {
    ...
    metricHelper.plusOne(DB_LATEST_STRATEGY_TVL_WRITE_SUCCESS);
  } catch (err) {
    console.error(`Latest Strategy TVL write failed with error:  ${err}`);
    metricHelper.plusOne(DB_LATEST_STRATEGY_TVL_WRITE_FAILURE);
  }
}
...
async function getPositionInfos(
  ...
  try {
    ...
    console.error(`Failed to query Terra Hive with error: ${error}.`);
    metricHelper.plusOne(BATCH_GET_POSITION_INFO_FAILURE);
    metricHelper.plusOne(HIVE_QUERY_ERROR);
    return;
  }
...
try {
  ...
  if (process.env.NODE_ENV === "production") {
    await metricHelper.publishMetrics(
      blockchain_network,
      "ApertureDataCollector"
    );
  } else {
    console.log("Skip publishing metrics for dev env. See metrics below.");
    console.dir(metricHelper.getRawMetrics(), { depth: null });
  }
